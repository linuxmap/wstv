 ...
 */
typedef struct{
	int bEnableTimeout; //启用超时机制。用于客户端，用于回调方式返回
...
//private
	...
	//for timeout
	PRIVATE int bEnableTimeout;//是否启用超时回调
	PRIVATE void *timeoutList; // list of sent event, used for timeout
	PRIVATE void *timeoutLock;
	PRIVATE void *timeoutThread;
	PRIVATE int bRunning;
}grpc_t;
...
/**
 *@brief 请勿关心，自动生成的代码使用。用于客户端，将#grpc_t::root 变成字符串发送出去
 */
int grpc_c_send(grpc_t *grpc);

/**
 *@brief 请勿关心，自动生成的代码使用。用于客户端，接收数据，并将收到的信息，变成#grpc_t::root
 */
