...
{
    TIME_T time_utc;
    INT no_use_time_zone;
    OPERATE_RET ret = tuya_ipc_get_service_time(&time_utc, &no_use_time_zone);
...
/*
用户可以设置Linux开发板时间（settimeofday），也可以不设置，每次都调用tuya_ipc_get_service_time来获取最新时间。
需要注意的是如果设置了Linux开发板时间，需要调用tuya_ipc_set_service_time来告知SDK。
*/
    struct timeval tv;
    tv.tv_sec = time_utc;
    tv.tv_usec = 0;
...
OPERATE_RET IPC_APP_Init_SDK(WIFI_INIT_MODE_E init_mode, CHAR *p_token)
{
    ...
    strcpy(env.p2p_id, s_mgr_info.p2p_id);
    strcpy(env.dev_sw_version,  s_mgr_info.dev_sw_version);
    strcpy(env.dev_serial_num,  "ipc_wifi_tutk");
    ...
    {
        strcpy(dev_info.wifi_init_cfg.debug_cfg.token, p_token);
    }

    tuya_ipc_init_sdk(&env);
    tuya_ipc_start_sdk(&dev_info);

    TUYA_APP_Enable_P2PTransfer(TRUE, s_mgr_info.max_p2p_user);

    TUYA_APP_Enable_EchoShow(TRUE);
    return OPRT_OK;
}
