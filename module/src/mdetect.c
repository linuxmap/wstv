...
static void _mdetect_process(void *param)
{
	...
	while (group.running)
	{
		...
		if(!ignore_flag && mdlist[chID].bOutVMS)
		{
			...
			//utl_iconv_gb2312toutf8("video-视频", alarmInfo.type, sizeof(alarmInfo.type));
			//utl_iconv_gb2312toutf8("移动侦测报警", alarmInfo.subtype, sizeof(alarmInfo.subtype));
			utl_iconv_gb2312toutf8("移动侦测报警", alarmInfo.pir_code, sizeof(alarmInfo.pir_code));
