...
void Remote_play_history_video(int channelno,char* filename,int starttime,int timeoffset)
{
	...
	len = strlen(filename);
	lenFlag = strlen(FILE_FLAG);
	if(strcmp(&filename[len-lenFlag], FILE_FLAG) != 0)
	{
		printf("only support mp4 file\n");
		goto play_err;
	}
	...
	memset(player, 0, sizeof(RemotePlayer_t));
	//player->clientId = clientId;
	//player->nConnectionType = nConnectionType;
	player->speed = 1.0;
	player->running = TRUE;
	player->frameRate = 25;//先设置一个初始值
	...
	player->setPosInFrame = timeoffset;
	...
	g_player = player;
	//创建线程,分离的
	ret = pthread_create_detached(&player->thread, NULL, (void *)_remote_player_mp4, (void *)player);
	if (ret != 0)
	{
		printf("create thread Failed: %d,  %d,%d,%d\n", ret, EAGAIN, EINVAL, EPERM);
		if(player->fp!=NULL)
		{
			if(player->bMp4)
				MP4_Close_Read(player->fp, &player->mp4Info);
