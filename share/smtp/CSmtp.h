...
#include <assert.h>
	...
	#define OutputDebugStringA(buf) //do {printf("%s %d   ", __FILE__, __LINE__);printf("%s", buf);}while(0)
...
public:
	...
	int testSMTP();
	int SendDirect(const char *servername
			, unsigned short port
			, const char *security
			, const char *login
			, const char *passwd
...
private:	
	...
	char *SendBuf;
	char *RecvBuf;
	std::string m_sCharset;		//charset of email,add by fang
