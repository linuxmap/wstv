...
{
	{command_INIT,          0,     15,  220, ECSmtp::SERVER_NOT_RESPONDING},
	{command_EHLO,          60,  60,  250, ECSmtp::COMMAND_EHLO},
	{command_AUTHPLAIN,     60,  60,  334, ECSmtp::COMMAND_AUTH_PLAIN},
...
Command_Entry* FindCommandEntry(SMTP_COMMAND command)
{
	...
	unsigned int nCommandCount = sizeof(command_list)/sizeof(command_list[0]);
	for(unsigned int i = 0; i < nCommandCount; ++i)
	{
...
void CSmtp::Send()
{
	unsigned int i,rcpt_count,res,FileId, nForCount;
	char FileBuf[55], FileName[255];
	FILE* hFile = NULL;
	...
	try{
		...
		nForCount = CCRecipients.size();
		for(i=0;i<nForCount;i++)
		{
		...
		TotalSize = 0;
		unsigned int nAttachmentsSize = Attachments.size();
		for(FileId=0;FileId<nAttachmentsSize;FileId++)
		{
			...
			// sending the file:
			if ((TotalSize >> 10) > (MSG_SIZE_IN_MB << 10))
			...
			else
			{
				...
				MsgPart = 0;
				unsigned int nCount = FileSize / 54 + 1;
				for(i=0;i<nCount;i++)
				{
					res = fread(FileBuf,sizeof(char),54,hFile);
					if (!MsgPart)
						strcpy(SendBuf, "\0");
					base64_encode_m(reinterpret_cast<const unsigned char*>(FileBuf),res,SendBuf);
					strcat(SendBuf,"\r\n");
					MsgPart += res + 2;
...
//	bool conected=false;
	...
	try
	{
		...
		if((sockAddr.sin_addr.s_addr = inet_addr(szServer)) == INADDR_NONE)
		{
			...
			gethostbyname_r(szServer, &hostinfo, buffer, sizeof(buffer), &host, &res);
			if (host)
		...
		if(IsKeywordSupported(RecvBuf, "AUTH") == true)
		{
			...
			else if(IsKeywordSupported(RecvBuf, "PLAIN") == true)
			{
				...
				unsigned int nSendBufLen = strlen(SendBuf);
				for(unsigned int i=0; i<nSendBufLen; i++)
				{
			...
			else if(IsKeywordSupported(RecvBuf, "DIGEST-MD5") == true)
			{
				...
				//send the response
				if(strstr(RecvBuf, "charset")>=0) sprintf(SendBuf, "charset=%s,username=\"%s\"", m_sCharset.c_str(), m_sLogin.c_str());
...
void CSmtp::FormatHeader(char* header)
{
	...
	{ // there is one or more attachments
		...
		strcat(SendBuf, m_sCharset.c_str());
...
/*
 * Setting the charset of mail.
 * author:fang
 */
void CSmtp::SetCharset(const char *charset)
{
	m_sCharset.erase();
	m_sCharset.insert(0, charset);
}
...
void CSmtp::SayHello()
{
	...
	sprintf(SendBuf, "EHLO %s\r\n","ubuntu");// GetLocalHostName()!=NULL ? m_sLocalHostName.c_str() : "domain");
	SendData(pEntry);
	ReceiveResponse(pEntry);
	m_bConnected=true;
}
...
int CSmtp::testSMTP()
{
	...
	try{
		SetSenderName("lfx1jovision.com");
		AddAttachment("abc");
	}
	catch(const ECSmtp &e)
	{
...
int CSmtp::SendDirect(const char *servername
		, unsigned short port
		, const char *security
		, const char *login
		, const char *passwd
		...
		)
{
	...
	{
		if (servername)
			SetSMTPServer(servername, port);
		if (!security)
			security = "none";
		CSmtp::security_type sec = CSmtp::NO_SECURITY;
		if (strcmp(security, "none") == 0)
			sec = CSmtp::NO_SECURITY;
		else if (strcmp(security, "tls") == 0)
			sec = CSmtp::USE_TLS;
		else if (strcmp(security, "ssl") == 0)
			sec = CSmtp::USE_SSL;
		SetSecurityType(sec);
		if (login)
		{
		...
		SetXMailer("JOVISION IPC (v8.01) Professional");
		...
		if (attachments)
		{
			string attachs = attachments;
			{
				split(attachs, spite, vectors);
				vector<string>::iterator iter;
				for( iter = vectors.begin(); iter != vectors.end(); iter++ )
				{
					//cout << *iter << "\n";
					AddAttachment(iter->c_str());
				}
			}
		}
		...
		if (recipients)
		{
			string recips = recipients;
			vectors.clear();
			split(recips, spite, vectors);
			vector<string>::iterator iter;
			for( iter = vectors.begin(); iter != vectors.end(); iter++ )
			{
				int nend = iter->find_first_of("@");
				if(nend == 0 || nend == (int)iter->length()-1||nend == -1)
				{
					printf("recipients format isn't right! %s\n", iter->c_str());
					return -1;
  				}
				else
				{
					AddRecipient(iter->c_str());
				}
			}
		}
	...
	{
		printf("llll\n");
		printf("GetErrorNum(void): %d\n", e.GetErrorNum());
		cout << "Error: " << e.GetErrorText().c_str() << ".\n";
		return (int)e.GetErrorNum();
	}
	cout << "End sending...\n";
