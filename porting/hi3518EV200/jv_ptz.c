...
static int src_self_check(int channel)
{
	...
	for (i = 0; i < nCheckTimes; ++i)
	{
		...
		{
			/*向左下转*/
			ioctl(sPtzParam.fd, MOTOR_SET_LR_STEP, sPtzParam.maxLRStep - 1);
			ioctl(sPtzParam.fd, MOTOR_SET_UD_STEP, 0);
			ioctl(sPtzParam.fd, MOTOR_SET_DIRECT, MOTOR_DIRECT_LEFT | MOTOR_DIRECT_DOWN);
		...
		{
			/*向右上转*/
			ioctl(sPtzParam.fd, MOTOR_SET_DIRECT, MOTOR_DIRECT_RIGHT | MOTOR_DIRECT_UP);
			if (ioctl(sPtzParam.fd, MOTOR_SET_START, 0) == 0)
			{
				__check_done();
			}
...
int jv_ptz_init()
{
	...
	if(!gp.bFactoryFlag)
	{
		ioctl(sPtzParam.fd, MOTOR_SET_LR_STEP, 0x8000);
		ioctl(sPtzParam.fd, MOTOR_SET_UD_STEP, 0x8000);
		sPtzParam.bAfterSelfCheck = TRUE;
	}
