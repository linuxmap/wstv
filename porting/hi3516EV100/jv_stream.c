...
static void __build_vpss_venc_size_vi(int vencw, int vench, SIZE_S *vpssSize, VENC_CROP_CFG_S *stGrpCropCfg)
{
	...
	expw = JV_ALIGN_CEILING(realvi_h * vencw / vench, 2);
	exph = JV_ALIGN_CEILING(realvi_w * vench / vencw, 2);

	if (expw - realvi_w >= 32)
	{
		stGrpCropCfg->bEnable = TRUE;
		stGrpCropCfg->stRect.s32X = 0;//JV_ALIGN_CEILING((expw - vencw)/2,16);
		stGrpCropCfg->stRect.s32Y = JV_ALIGN_CEILING((realvi_h - exph)/2, 2);
		stGrpCropCfg->stRect.u32Width = realvi_w;
		stGrpCropCfg->stRect.u32Height = exph;
		vpssSize->u32Width = realvi_w;//2 * stGrpCropCfg->stRect.s32X + vencw;//JV_ALIGN_CEILING(expw, 16);
		vpssSize->u32Height = realvi_h;
	}
	else if (exph - realvi_h > 10)
	{
		stGrpCropCfg->bEnable = TRUE;
		stGrpCropCfg->stRect.s32X = JV_ALIGN_CEILING((realvi_w-expw)/2,16);
		stGrpCropCfg->stRect.s32Y = 0;
		stGrpCropCfg->stRect.u32Width = expw;
		stGrpCropCfg->stRect.u32Height = realvi_h;
		vpssSize->u32Width = realvi_w;
		vpssSize->u32Height = realvi_h;
	}
	else
	{
...
//		framerate = srcFramerate;
	...
	******************************************/
	...
	case PT_H264:
	{
		...
		stH264Attr.u32BufSize  = JV_ALIGN_CEILING((int)(stPicSize.u32Width * stPicSize.u32Height * 3 / 4),64);/*stream buffer size*/
		...
		if(SAMPLE_RC_CBR == enRcMode)
		{
			...
			stH264Cbr.u32SrcFrmRate     = fvif;/* input (vi) frame rate */
		...
		else if (SAMPLE_RC_FIXQP == enRcMode)
		{
			...
			stH264FixQp.u32SrcFrmRate = fvif;
	...
	case PT_H265:
	{
		...
		stH265Attr.u32BufSize  = JV_ALIGN_CEILING((int)(stPicSize.u32Width * stPicSize.u32Height * 3 / 4),64);/*stream buffer size*/
		...
		if(SAMPLE_RC_CBR == enRcMode)
		{
			...
			stH265Cbr.u32SrcFrmRate     = fvif;/* input (vi) frame rate */
		...
		else if (SAMPLE_RC_FIXQP == enRcMode)
		{
			...
			stH265FixQp.u32SrcFrmRate = fvif;
		...
		else if (SAMPLE_RC_AVBR == enRcMode)
		{
			...
			VENC_ATTR_H265_AVBR_S stH265Vbr;
			stH265Vbr.u32Gop = nGop;
			stH265Vbr.u32StatTime = (nGop+framerate-1)/framerate;
			stH265Vbr.u32SrcFrmRate = fvif;
			stH265Vbr.fr32DstFrmRate = framerate;
			stH265Vbr.u32MaxBitRate = bitrate;
			memcpy(&stVencChnAttr.stRcAttr.stAttrH265AVbr, &stH265Vbr, sizeof(VENC_ATTR_H265_VBR_S));
		}
		else
		{
			return HI_FAILURE;
	...
	//if(nGop>=NGOP_FRAMERATE*framerate*2)
	//先不管smartP
	if (0)
	{
		//SmartP
		stVencChnAttr.stGopAttr.enGopMode = VENC_GOPMODE_SMARTP; 
		stVencChnAttr.stGopAttr.stSmartP.u32BgInterval = nGop; // 25fps, 30seconds 
		stVencChnAttr.stGopAttr.stSmartP.s32BgQpDelta = 7; 
		stVencChnAttr.stGopAttr.stSmartP.s32ViQpDelta = 2; 
		//stVencChnAttr.stRcAttr.stAttrH264Vbr.u32Gop= fvif*2;   // virtual I interval 
		//stVencChnAttr.stRcAttr.stAttrH264Vbr.u32StatTime = 30;   // 30 second
		//stVencChnAttr.stRcAttr.u32Gop = 30;   // virtual I interval 
		//stVencChnAttr.stRcAttr.u32StatTime = 40;   // 40 second

		//
		printf(">>>>>>>>>>>>SmartP,   nGop = %d \n", nGop);
	}
	else
	{
		stVencChnAttr.stGopAttr.enGopMode  = VENC_GOPMODE_NORMALP;
		stVencChnAttr.stGopAttr.stNormalP.s32IPQpDelta = 0;
		printf(">>>>>>>>>>>>NormalP,   nGop = %d \n", nGop);
	}
	...
	if(enType == PT_H264)
	{
		HI_U32 thrdI[RC_TEXTURE_THR_SIZE] = {0, 0, 0, 0, 3,3,5,5,8,8,8,15,20,20,25,25};
		HI_U32 thrdP[RC_TEXTURE_THR_SIZE] = {0, 0, 0, 0, 3,3,5,5,8,8,8,15,20,20,25,25};
...
//		rcPara.stParamH264VBR.u32MinIQP = 3;
	...
	else if(SAMPLE_RC_AVBR == enRcMode)
	{
		...
		{
			rcPara.stParamH265AVbr.u32MinIQp = attr->minQP;
			rcPara.stParamH265AVbr.u32MaxIQp = attr->maxQP;
			rcPara.stParamH265AVbr.u32MinQp = attr->minQP;
			rcPara.stParamH265AVbr.u32MaxQp = attr->maxQP;
			rcPara.stParamH265AVbr.u32MaxStillQP = 40;
			rcPara.stParamH265AVbr.s32MinStillPercent = 50;
			// rcPara.stParamH265AVbr.s32IPQPDelta = 0;
		}
	}
	else
	{
...
HI_S32 SAMPLE_COMM_VENC_BindVpss(VENC_CHN VencChn,VPSS_GRP VpssGrp,VPSS_CHN VpssChn)
{
	...
	else
	{
		...
		pthread_create_normal(&Args[VencChn].pid, NULL, _Vpss_To_Venc, &Args[VencChn]);
	}
#else
	MPP_CHN_S stSrcChn;
	MPP_CHN_S stDestChn;
...
int jv_stream_start(int channelid)
{
	...
	if(VI_WIDTH*VI_HEIGHT == 1920*1080)			//200W的在VPSS修改帧率
	{
		...
		float fvif;
		jv_sensor_get_b_low_frame(0, &fvif, NULL);
	...
	int bRotate = 0;
	HI_MPI_VPSS_GetRotate(VpssGrp, 0, &rotate);	//三个通道同时旋转固定从0通道获取参数就可以了
	...
	if(VI_CROP_ENABLE)
	{
		if(attr->width*attr->height >= VI_CROP_W*VI_CROP_H)
		{
			ststatus[channelid].attr.bRectStretch = 1;
		}
	}
	...
	if (VpssChn[channelid] <= 2)
	{
		...
		if (bRotate)
		{
			vpssMode.u32Width = size.u32Height;
			vpssMode.u32Height = size.u32Width;
		}
...
int jv_stream_read(int channelid, jv_frame_info_t *info)
{
	...
	for (i=0; i< stStream.u32PackCount; i++)
	{
		...
		if(ststatus[channelid].attr.vencType == JV_PT_H264)
		{
			...
			else if(stStream.stH264Info.enRefType == BASE_PSLICE_REFTOIDR)
			{
				//printf("virtual I.  enH264EType = %d\n", stStream.pstPack[i].DataType.enH264EType);
				info->frametype = JV_FRAME_TYPE_VI;
			}
		}
		else if(ststatus[channelid].attr.vencType == JV_PT_H265)
		{
			if(stStream.pstPack[i].DataType.enH265EType == H265E_NALU_IDRSLICE)
			{
				info->frametype = JV_FRAME_TYPE_I;
			}
			else if(stStream.stH265Info.enRefType == BASE_PSLICE_REFTOIDR)
			{
				printf("virtual I.  enH265EType = %d\n", stStream.pstPack[i].DataType.enH265EType);
				info->frametype = JV_FRAME_TYPE_VI;
			}
		}
...
int jv_stream_set_attr(int channelid, jv_stream_attr *attr)
{
	...
	if (ststatus[channelid].bOpened)
	{
		...
		{
			if(hwinfo.bSupportAVBR && channelid==0)
			{
				if(ststatus[channelid].attr.vencType==JV_PT_H264)
				{
				...
				{
					stAttr.stRcAttr.stAttrH265AVbr.u32MaxBitRate = ststatus[channelid].attr.bitrate;
					stAttr.stRcAttr.stAttrH265AVbr.u32Gop = nGop;
					stAttr.stRcAttr.stAttrH265AVbr.u32StatTime = (nGop+framerate-1)/framerate;
					stAttr.stRcAttr.stAttrH265AVbr.u32SrcFrmRate = rcSrcFrameRate;
					stAttr.stRcAttr.stAttrH265AVbr.fr32DstFrmRate = rcDstFrameRate;

					rcPara.stParamH265AVbr.u32MinQp = ststatus[channelid].attr.minQP;
					rcPara.stParamH265AVbr.u32MinIQp = ststatus[channelid].attr.minQP;
					rcPara.stParamH265AVbr.u32MaxQp =  ststatus[channelid].attr.maxQP;
					rcPara.stParamH265AVbr.u32MaxIQp =  ststatus[channelid].attr.maxQP;
				}
			}
			else
			{
				...
				{
					stAttr.stRcAttr.stAttrH265Vbr.u32MaxBitRate = ststatus[channelid].attr.bitrate;
					stAttr.stRcAttr.stAttrH265Vbr.u32Gop = nGop;
					stAttr.stRcAttr.stAttrH265Vbr.u32StatTime = (nGop+framerate-1)/framerate;
					stAttr.stRcAttr.stAttrH265Vbr.u32MaxQp =  ststatus[channelid].attr.maxQP;
					stAttr.stRcAttr.stAttrH265Vbr.u32MinQp =  ststatus[channelid].attr.minQP;
					stAttr.stRcAttr.stAttrH265Vbr.u32SrcFrmRate = rcSrcFrameRate;
					stAttr.stRcAttr.stAttrH265Vbr.fr32DstFrmRate = rcDstFrameRate;
				}
			}
		}
		else if (ststatus[channelid].attr.rcMode == JV_VENC_RC_MODE_CBR)
		{
			...
			{
				stAttr.stRcAttr.stAttrH265Cbr.u32BitRate = ststatus[channelid].attr.bitrate;
				stAttr.stRcAttr.stAttrH265Cbr.u32Gop = nGop;
				stAttr.stRcAttr.stAttrH265Cbr.u32StatTime = (nGop+framerate-1)/framerate;
				stAttr.stRcAttr.stAttrH265Cbr.u32FluctuateLevel = 0; /* average bit rate */
				stAttr.stRcAttr.stAttrH265Cbr.u32SrcFrmRate = rcSrcFrameRate;
				stAttr.stRcAttr.stAttrH265Cbr.fr32DstFrmRate = rcDstFrameRate;
			}
		}
		else
		{
...
int jv_stream_switch_lowbitrate(int channelid, unsigned int bitrate)
{
	...
	{
		HI_MPI_VENC_GetRcParam(channelid, &rcPara);
		if(ststatus[channelid].attr.vencType == JV_PT_H265)
		{
			rcPara.stParamH265AVbr.u32MinIQp = 31;
			rcPara.stParamH265AVbr.u32MaxIQp = 51;
			rcPara.stParamH265AVbr.u32MinQp = 33;
			rcPara.stParamH265AVbr.u32MaxQp = 51;
			rcPara.stParamH265AVbr.u32MaxStillQP = 44;
			rcPara.stParamH265AVbr.s32MinStillPercent = 25;
		}
		else
		{
