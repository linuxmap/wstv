...
{
	//sharpen
	ISP_ARRAY_U8 SharpenAltD[7];
	ISP_ARRAY_U16 SharpenUd[7];
	ISP_ARRAY_U8	TextureThr[7];
	ISP_ARRAY_U8	DetailCtrl[7];
	//降噪
	ISP_ARRAY_U16 NoiseCoringWeight[7];
	ISP_ARRAY_U8 SBS3[7];
	ISP_ARRAY_U8 SDS3[7];
	//拖影时域
	ISP_ARRAY_U8 MATH[7];
...
typedef struct 
{
	unsigned char u8Fluency;  //用户设置流畅度
	unsigned char u8Definition; //清晰度  
	unsigned char u8DeSmear;    //拖影抑制强度
	unsigned char u8DeNoise;    //降噪强度
	unsigned char u8framelevel ;//降帧状态级数，包括25帧默认为3

}COMM_ISP_PARAMS_LEVEL;
...
/**
 *@brief 打开或者关闭宽动态功能，兼容sensor 和数字宽动态，即DRC
 *@param 是否开启WDR
 *@retval 0  :设置成功，非0  失败
 */
