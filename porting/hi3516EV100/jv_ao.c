        ...
        AIO_ATTR_S *pstAioAttr, AUDIO_SAMPLE_RATE_E enInSampleRate, HI_BOOL bResampleEn)
{
	...
	*/
		...
		stAoVqeCfg.u32OpenMask = AO_VQE_MASK_AGC | AO_VQE_MASK_ANR | AO_VQE_MASK_HPF;
...
int jv_ao_stop(int channelid)
{
	...
	HI_MPI_AO_GetPubAttr(DEV_AO, &stAioAttr);
	int i = 0;
	for (i = 0; i < stAioAttr.u32ChnCnt; i++)
	{
		CHECK_RET(HI_MPI_AO_DisableChn(DEV_AO, i));
	}
...
int jv_ao_mute(BOOL bMute)
{
	...
	{
		utl_system("insmod /home/ipc_drv/extdrv/AW8733A.ko Group=5 Index=4 MuxReg=0x120400C4 MuxVal=0 OnVal=1");
		usleep(1000*1000);
	}
